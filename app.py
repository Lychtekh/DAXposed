...
def load_models():
    ...
    # Generar todos los umbrales de -10% a 10% con paso de 1%
    umbrales = [i/100 for i in range(-10, 11)]  # -0.10 a 0.10 con paso 0.01
    
    modelos_config = {}
    for periodo in ['1 Semana', '1 Mes', '3 Meses', '6 Meses', '1 Año']:
        modelos_config[periodo] = {}
        for threshold in umbrales:
            modelos_config[periodo][threshold] = {
                'path': 'modelNOW.joblib', 
                'dim': 787
            }
                    ...
                    try:
                        ...
                        <div style="background: rgba(26, 35, 50, 0.6); padding: 1.5rem; border-radius: 10px; border-left: 4px solid {interpretation_color};">
                            ...
                            <p style="color: #aaa; font-size: 1rem; margin: 0;">
                                ...
                                🎚️ <strong>Sensibilidad:</strong> {threshold}
                            </p>
                        </div>
                        """, unsafe_allow_html=True)
